...
def attach_ducklake(db__name: str = '') -> duckdb.DuckDBPyConnection:
    """DuckDB connection for DuckLake."""

    # Initialize DuckDB connection
    db = duckdb.connect(db__name)

    # Setup postgres as catalog backend for DuckLake
    ...
    # Setup R2 as storage backend for DuckLake
    ...
    db.execute(f"""
        CREATE SECRET(
            ...
            ACCOUNT_ID '{os.getenv("R2__ACCOUNT_ID")}'
        );
    """)

    # Attach the DuckLake
    db.install_extension("ducklake")
    db.execute(f"ATTACH 'ducklake:postgres:dbname={os.getenv("PG__DATABASE")} host={os.getenv("PG__HOST")}' AS ducklake (DATA_PATH 'r2://{os.getenv("R2__BUCKET")}', ENCRYPTED);")
    db.execute("USE ducklake;")
...
def load_adventure_works(env) -> None:
    ...
    ducklake = attach_ducklake()
    
    pipeline = dlt.pipeline(
        pipeline_name="adventure_works",
        destination=dlt.destinations.duckdb(ducklake),
        dataset_name="das",
        progress="enlighten",
    ...
    ducklake.sql("SHOW ALL TABLES;").show()
    ducklake.close()
